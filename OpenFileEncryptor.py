...
def check_for_updates(version):
    try:
        global download_url, updater_app, start_update_gui, update_gui
...
def create_new_settings_file():
    ...
    try:
        ...
        config['Settings'] = {'Language': 'English', 'Logs': 'off', 'CustomThemeName': thememenu, 'CustomThemePath': 'None'}
    except:
        config['Settings'] = {'Language': 'English', 'Logs': 'off', 'CustomThemeName': 'Standard theme', 'CustomThemePath': 'None'}
...
def read_settings():
    global Language, Language_restore, Logs, thememenu_lan, CustomThemeName, CustomThemePath
    config = configparser.ConfigParser()
    try:
...
def switch_language():
    global select_dir_button_enc_lan, path_entry_enc_lan, pin_entry_enc_lan, encryption_button_lan, check_box_lan, check_box_log_lan, select_dir_button_dec_lan, new_update_label_lan, button_update_lan, button_close_lan, updating_label_lan,path_entry_dec_lan, pin_entry_dec_lan, decryption_button_lan, seg_button_lan_1, seg_button_lan_2, seg_button_lan_3, save_settings_lan, save_key_button_lan, upload_from_file_entry_enc_lan, text_enc
    if Language == 'English':
...
def save_settings():
    with open('Settings.cfg', 'wb'):
        pass
    global Language_save, Logs_save
    ...
    if 'Language_save' not in globals():
        ...
        if thememenu.get() == thememenu_lan:
            config['Settings'] = {'Language': Language, 'Logs': Logs_save, 'CustomThemeName': thememenu_lan, 'CustomThemePath': 'None'}
        else:
            config['Settings'] = {'Language': Language, 'Logs': Logs_save, 'CustomThemeName': thememenu.get(), 'CustomThemePath': full_path}
    else:
        config['Settings'] = {'Language': Language_save, 'Logs': Logs_save}
        if thememenu.get() == thememenu_lan:
            config['Settings'] = {'Language': Language_save, 'Logs': Logs_save, 'CustomThemeName': thememenu_lan, 'CustomThemePath': 'None'}
        else:
            config['Settings'] = {'Language': Language_save, 'Logs': Logs_save, 'CustomThemeName': thememenu.get(), 'CustomThemePath': full_path}
...
def settings():
    global save, close, language_menu, check_box_log, check_box_log_var, thememenu, copyright_label_1, copyright_label_2
...
def console_enc():
    ...
    try:
        ...
        pass
    textbox = customtkinter.CTkTextbox(app, fg_color='black', text_color='#2fa572',state = DISABLED,width=400, height=170)
